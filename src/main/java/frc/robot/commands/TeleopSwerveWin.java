...
public class TeleopSwerveWin extends CommandBase {    
     ...
     * @param quarterSpeed
     */
    public TeleopSwerveWin(CommandSwerveDrivetrain swerve, DoubleSupplier xSup, DoubleSupplier ySup, 
    ...
    public void execute() {
        ...
        if(rotateWithButton){
            ...
            rotationVal = m_thetaController.calculate((MathUtil.inputModulus(m_Swerve.getState().Pose.getRotation().getDegrees(), -180, 180)), m_thetaController.getSetpoint());
        ...
        m_Swerve.setControl(drive.withVelocityX(xVal * Constants.Swerve.MaxSpeed) // Drive forward with
        // negative Y (forward)
            .withVelocityY(yVal * Constants.Swerve.MaxSpeed) // Drive left with negative X (left)
            .withRotationalRate(rotationVal * Constants.Swerve.maxAngularVelocity) // Drive counterclockwise with negative X (left)
        );
